!=======================================================================
!  Neural-Network Based Pressure Gradient Error (PGE) Correction
...
module nn_pge_correction
  ...
  ! Hyper-parameters
  !---------------------------------------------------------------------
  integer, parameter :: NEPOCHS    = 500       ! training epochs
  ...
  real, save :: tgt_mean          ! target (PGE)
  real, save :: tgt_std           ! target (PGE)

  !---------------------------------------------------------------------
  ! Runtime
  !---------------------------------------------------------------------
  real,    save :: alpha_pge = 1.0
...
!=======================================================================
  pure real function forward_pass(x_raw) result(corr)
    ...
    corr = ( dot_product(W3(1,:), h2v) + b3(1) ) * tgt_std + tgt_mean
...
!!!=====================================================================
    ...
    if (my_rank == 0) then
      write(6,'(/,A)')         &
        '  ========================================================'
      write(6,'(A)')           &
        '   NN_PGE  TRAINING  (--IN PARALLEL MODE-- (--wait i finish soon--))'
...
!!!----------------------------------------------------------------------
    ...
    ! rho_in is there (0:Lm_in+1, 0:Mm_in+1, Nlev) so that sm-loop did. ok.
    i_lo = 0
    i_hi = Lm_in + 1
    j_lo = 0
    j_hi = Mm_in + 1
...
!!!----------------------------------------------------------------------
    ...
    !--- Early exit if no samples collected (no mean no) -----------------
...
!!!------------------------------------------------------------------------
    ...
    !--- Target (Yl) normalisation --------------------------------------
    do s = 1, nsamp_loc
      loc_sum(1) = loc_sum(1) + Yl(s)
      loc_sq (1) = loc_sq (1) + Yl(s)**2
    end do
    ! Reuse first element -----------------------------------------------
    loc_sum(1) = sum(Yl(1:nsamp_loc))
    loc_sq (1) = sum(Yl(1:nsamp_loc)**2)
    call MPI_Allreduce(loc_sum(1), glb_sum(1), 1, &
                       MPI_DOUBLE_PRECISION, MPI_SUM, mpi_comm, mpi_err)
    call MPI_Allreduce(loc_sq (1), glb_sq (1), 1, &
                       MPI_DOUBLE_PRECISION, MPI_SUM, mpi_comm, mpi_err)
    tgt_mean = glb_sum(1) / glb_nsamp_r
    tgt_std  = sqrt( max( glb_sq(1)/glb_nsamp_r - tgt_mean**2, 0.0 ) &
                   + 1.0e-8 )
